...
import {
  careerProcessStepAllowsAttachments,
  getCareerApplicationStep,
...
import {
  buildCareerApplicationStepMediaPath,
...
function resolveStep(applicationSlug: string, stepSlug: string) {
  const ref = getCareerApplicationStep(applicationSlug, stepSlug);
  if (!ref || !careerProcessStepAllowsAttachments(ref.step)) return null;
  return ref;
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const { slug, step } = await context.params;
  const ref = resolveStep(slug, step);
  if (!ref) {
    return NextResponse.json({ error: "단계를 찾을 수 없습니다." }, { status: 404 });
  }

  const directory = buildCareerApplicationStepMediaPath(slug, step);
