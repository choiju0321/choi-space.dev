...
import {
  careerHubEntryAllowsAttachments,
  getCareerHubEntry,
  isCareerMediaSpace,
...
import {
  buildCareerEntryMediaPath,
  sanitizeCareerAttachmentFileName,
} from "@/lib/media/career-paths";
import { hasWriteSession } from "@/lib/write/auth";

export const dynamic = "force-dynamic";

type RouteContext = {
  params: Promise<{ space: string; entry: string }>;
};

function resolveEntry(spaceRaw: string, entrySlug: string) {
  if (!isCareerMediaSpace(spaceRaw)) return null;
  const entry = getCareerHubEntry(spaceRaw, entrySlug);
  if (!entry || !careerHubEntryAllowsAttachments(entry)) return null;
  return { space: spaceRaw, entry };
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const { space, entry } = await context.params;
  const ref = resolveEntry(space, entry);
  if (!ref) {
    return NextResponse.json({ error: "항목을 찾을 수 없습니다." }, { status: 404 });
  }

  const directory = buildCareerEntryMediaPath(ref.space, ref.entry.slug);
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  let fileName = sanitizeCareerAttachmentFileName(preferred || file.name);
