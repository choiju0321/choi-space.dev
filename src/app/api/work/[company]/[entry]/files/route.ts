...
import {
  getWorkAttachable,
  workAttachableAllowsAttachments,
...
import {
  buildWorkEntryMediaPath,
  sanitizeWorkAttachmentFileName,
} from "@/lib/media/work-paths";
import { hasWriteSession } from "@/lib/write/auth";

export const dynamic = "force-dynamic";

type RouteContext = {
  params: Promise<{ company: string; entry: string }>;
};

function resolveEntry(companySlug: string, entrySlug: string) {
  const ref = getWorkAttachable(companySlug, entrySlug);
  if (!ref || !workAttachableAllowsAttachments(ref.entry)) return null;
  return ref;
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const { company, entry } = await context.params;
  const ref = resolveEntry(company, entry);
  if (!ref) {
    return NextResponse.json({ error: "항목을 찾을 수 없습니다." }, { status: 404 });
  }

  const directory = buildWorkEntryMediaPath(ref.company.slug, ref.entry.slug);
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  let fileName = sanitizeWorkAttachmentFileName(preferred || file.name);
